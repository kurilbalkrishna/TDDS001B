...
# Sidebar with project information and methodology
with st.sidebar:
...
# Create Nikkei closing price chart
fig = go.Figure()
fig.add_trace(go.Scatter(x=df["Date"], y=df["Close"], mode="lines", name="Close Price"))
fig.update_layout(xaxis_title="Date", yaxis_title="Price", height=400)

# Walk-forward validation results for both models
...
with tab2:
    ...
    # Allow user to select a custom date range
    ...
    # Filter price data based on selected dates
    filtered_df = df[
        (df["Date"].dt.date >= date_range[0]) &
        (df["Date"].dt.date <= date_range[1])
    ]

    # Create filtered price chart
...
with tab4:
    ...
    # Correlation values between model features
    correlation_data = {
    ...
    corr_df = pd.DataFrame(
        correlation_data,
        index=["Momentum_5", "Momentum_20", "Volatility_20"]
    )
