...
\subsection{Combinatorial Purged Cross-Validation (Supplementary)}

As a robustness check on the walk-forward result, a Combinatorial Purged
Cross-Validation (CPCV) analysis was additionally performed, following
Lopez de Prado (2018, Ch.~12). The sample was divided into $N=6$
contiguous groups, with $k=2$ groups tested jointly per split
($\binom{6}{2}=15$ splits), yielding $\phi = 5$ reconstructed,
full-coverage out-of-sample backtest paths. Purging (horizon of 5 days,
matching \texttt{Future\_Return\_5}) and embargo were applied identically
to the walk-forward analysis.

\begin{table}[h]
\centering
\begin{tabular}{lcc}
\toprule
Path & Regression $R^2$ & Random Forest $R^2$ \\
\midrule
1 & 0.0046  & 0.0014  \\
2 & 0.0002  & $-0.0134$ \\
3 & 0.0020  & $-0.0081$ \\
...
\bottomrule
\end{tabular}
\caption{$R^2$ across the 5 CPCV backtest paths.}
\label{tab:cpcv}
\end{table}

Table~\ref{tab:cpcv} shows Regression outperforming Random Forest on
every one of the 5 paths, in contrast to the walk-forward result, where
the two models were statistically indistinguishable. A paired $t$-test
across paths yields $t = 3.19$, $p = 0.033$, nominally significant at
the 5\% level. This result should be treated as suggestive rather than
confirmatory: with only 5 paths, the test has limited statistical power,
and CPCV paths are not fully independent of one another, since they are
reconstructed from a shared, finite set of underlying splits. We
nonetheless view the consistent direction of the effect across all 5
paths --- Regression matching or exceeding Random Forest in every
instance --- as corroborating evidence for the walk-forward finding that
the simpler model is, if anything, more reliable than the more flexible
one in this setting.

A CPCV-based Sharpe ratio distribution for the regression-based
long/flat strategy was also computed across the same 5 paths (mean
$-0.29$, range $-0.85$ to $-0.04$). All 5 paths were negative, reinforcing
the walk-forward conclusion that no tradeable edge survives realistic
evaluation once transaction costs are considered. 
