...
Neither model shows statistically significant predictive power once evaluated under walk-forward validation (paired t-test p = 0.46), and any small edge does not survive realistic transaction costs. An unconstrained Random Forest model is shown to overfit severely, underscoring the importance of regularization and rigorous multi-window validation over single-split evaluation. Training windows were purged of the last (horizon−1) observations at each split boundary to eliminate label-overlap leakage between train and test; verified negligible effect on results.
